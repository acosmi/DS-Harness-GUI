...
macOS arm64 与 x64 的 DMG 和 ZIP 在本地 Mac 上打包（`pnpm run desktop:package:mac`，或 candidate/stable 签名变体），并用 `pnpm run desktop:publish:mac` 上传到 identity 账本的 GitHub Release。未签名的 Windows NSIS 安装包通过针对同一 commit 派发 [desktop-windows-package.yml](../../../.github/workflows/desktop-windows-package.yml) 打包；该工作流的 Ubuntu 作业执行 Windows 上传。GitHub Release 上传要求工作树干净，并且不表示 stable 提升。
