...
**Scope**: the root CONTRIBUTING and BRAND_GUIDELINES documents, every non-vendor README except the explicitly excluded Chinese-first product landing page, and every active document under `.agents/notes/**`, `docs/**`, and `python/**`. README matching is case-insensitive on the basename and covers future directories without another manifest edit. Dependency and ignored build-output trees and the frozen `.agents/notes/archived/` tree are discovery exclusions, not evolving translation source.
...
- The root [README.md](../../README.md) — the Acosmi distribution repository uses this Chinese-first landing page and offers [README.en.md](../../README.en.md) as its optional English version; product landing pages do not use the upstream `.zh.md` pairing convention.
...
**Universal requirement**: every current or future document in scope must merge as a complete bilingual pair. [scripts/translation-pairing.manifest.json](../../scripts/translation-pairing.manifest.json) contains only explicit exclusions; there is no per-file rollout list or date cutoff.
