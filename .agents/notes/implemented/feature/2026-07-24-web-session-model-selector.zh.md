...
浏览器中的 `ModelDirectoryResolver` 为每个实时会话持有一个 `ModelDirectory`。其快照包含当前完整的 `ModelSelection`、分组目录、提供方失败记录、操作错误，以及 `idle`、`loading`、`ready`、`selecting`、`error` 状态。挂载时会预先填充触发器标签，此后每次打开菜单都会刷新目录。目录加载与选择使用彼此独立的代次，因此较早的目录响应不会覆盖较新结果，目录刷新也不能丢弃进行中的 `session.selectModel`；连接重置会先丢弃当前进程中的投影，再恢复 Host 选择。失败时保留先前的选择和可用分组。
