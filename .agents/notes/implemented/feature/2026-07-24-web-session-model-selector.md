...
The browser `ModelDirectoryResolver` owns one `ModelDirectory` per live session. Its snapshot contains the current complete `ModelSelection`, grouped catalog, provider failures, operation error, and `idle`/`loading`/`ready`/`selecting`/`error` state. Mounting primes the trigger label and each menu open refreshes the directory. Directory loads keep one epoch and selections keep another, so an older catalog response never overwrites a newer one and a catalog refresh cannot discard an in-flight `session.selectModel`; connection reset discards the process-local projection before restoring the Host selection. Failures retain the previous selection and usable groups.
