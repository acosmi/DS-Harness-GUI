...
每个拉取请求还会启动一个名称为 `windows node 24 / native complete` 的常规独立 `windows-native` 作业。规范仓库 `deepseek-ai/deepseek-harness` 使用组织自有的 `dsh-windows-2025-16core` 运行器，显式故障切换时改用其自托管 Windows 池；派生仓库则使用标准 `windows-2025`，因为组织自有标签不可移植。该作业为工作区符号链接启用开发人员模式，通过 `pnpm/action-setup` 提供仓库固定版本的 pnpm，在不传输 store 归档的情况下执行不可变安装，并在原生 PowerShell 下运行 `pnpm run check:ci:windows-complete`。门禁卡住时，120 分钟超时会为其设定上限，同时不把实测性能目标当作正确性截止时间。
...
**所有仓库都使用 GitHub 标准 `windows-2025` 运行器。** 该可移植镜像能可靠完成这份完整清单，但其 32 分钟的串行结果使自动原生信号的实用性远低于所选的 16 核运行器。它仍作为派生仓库的回退方案，因为较慢的真实结果优于因组织自有标签不可用而永久排队的作业。
...
维护者必须保留这些有意设计的执行拓扑：Wine 快照使用 Linux 安装加 hoisted 布局来触达 win32 二进制文件，而原生作业在规范仓库的组织自有 16 核 Windows 运行器或派生仓库的标准 Windows 镜像上使用不可变工作区。某一拓扑独有的失败必须依据这一区别分类，不得削弱或静默跳过。
