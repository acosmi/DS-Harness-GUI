...
原打包路径还允许 electron-builder 在没有识别到凭据环境时跳过公证；它会在生成目标前公证并 staple 应用，却不提交最终 DMG；签名检查也没有证明每个 Mach-O 只包含隔离构建的目标架构。DMG blockmap 会在 staple ticket 改变分发包字节前生成。跨架构部署还会在宿主 Node 进程中运行依赖生命周期脚本，导致 Koffi 选择宿主架构而非 pnpm 的 x64 目标，并尝试无效的原生编译。因此，命令成功不一定提供发行账本要求的完整证据，也可能在生成第二种架构前失败。
...
聚焦测试固定 release mode 解析、签名器中的精确证书选择、签名后指纹检查、受信签名分类、完整且互斥的凭据组、无生命周期脚本的目标部署、目标专属旧产物清理、staple 后 blockmap 重建、notarytool 参数构造、已接受提交解析与单一目标架构检查。最终 candidate 执行提供两种架构的 Apple submission id、Developer ID 与 Gatekeeper 结果、各格式 SHA-256 以及 ZIP 解包检查证据。
