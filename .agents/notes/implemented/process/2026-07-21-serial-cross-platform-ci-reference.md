...
The active serial references run on the self-hosted `vm-backup` (`serial / linux`) and `dsh-win-ci` (`serial / windows`) pools; the one remaining disabled hosted serial reference (`serial-macos`) uses `macos-latest`, and there is no standard-hosted `serial / linux` label. The required pull-request Windows job runs under Wine on `ubuntu-latest`, while the independent pull-request native job uses the canonical repository's hosted `dsh-windows-2025-16core` runner under normal operation, its self-hosted `[self-hosted, dsh-win-ci, windows]` pool under failover, or standard `windows-2025` in a derived repository (see the [failover runbook](2026-07-26-ci-failover-runbook.md)); it is absent from the required aggregate under the [dual Windows decision](2026-08-08-native-windows-pull-request-ci.md). Under the [required-CI decision](2026-07-23-portable-required-pull-request-ci.md), derived repositories use portable standard capacity for the required inventory, while the canonical repository keeps its measured larger-runner path and proven self-hosted failover. Higher-core hosted runners remain manual benchmarks because a correctness path must remain runnable without repository-external runner configuration.
