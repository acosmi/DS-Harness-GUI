...
The portable reference uses GitHub's standard `ubuntu-latest`, `macos-latest`, and `windows-2025` labels. The required pull-request Windows job runs under Wine on `ubuntu-latest`, while the independent pull-request native job uses the canonical repository's hosted `dsh-windows-2025-16core` runner under normal operation, its self-hosted `[self-hosted, dsh-win-ci, windows]` pool under failover, or standard `windows-2025` in a derived repository (see the [failover runbook](2026-07-26-ci-failover-runbook.md)); it is absent from the required aggregate under the [dual Windows decision](2026-08-08-native-windows-pull-request-ci.md). When enabled, `serial / windows` remains a second complete, unsharded native-kernel oracle. Under the [required-CI decision](2026-07-23-portable-required-pull-request-ci.md), derived repositories use portable standard capacity for the required inventory, while the canonical repository keeps its measured larger-runner path and proven self-hosted failover.
