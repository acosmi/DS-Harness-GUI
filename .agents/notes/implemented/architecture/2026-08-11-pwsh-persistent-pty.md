...
One backend, two dialects: `shellDialect: 'bash' | 'pwsh'` (default `'bash'`, existing deployments byte-identical). The effective `shellPath`/`shellArgs` resolve per dialect (bash `/bin/bash --noprofile --norc -i`; pwsh through the shared `dsh-pwsh-local` resolver with `-NoLogo -NoProfile`, keeping the interactive host for child REPLs). The child environment drops the bash-only `PS1`/`PROMPT_COMMAND` markers and adds `NO_COLOR` for pwsh. A pwsh session answers PSReadLine's standard cursor-position requests with a fixed logical position, waits for initial interactive output, and then submits the encoding and prompt bootstrap once. Silence-settled follow-up sends are empty observations; publication requires the marker-owned prompt or a complete controlled-prompt line tail within one shared startup deadline. Both dialects emit the same BEL-terminated OSC `133;D;` marker and share `PROMPT_MARKER_PREFIX`, `CONTROLLED_PROMPT`, and exact-tail readiness; the marker remains an unconsumed readiness signal with no model-notification channel. The [pwsh readiness correction](../bug-fix/2026-08-20-pwsh-stdin-wait-readiness.md) owns the terminal-response and completion-evidence rationale.
...
**PowerShell readiness uses prompt or silence on every platform.** A `pwsh`-dialect session does not accept thread-level stdin waits as command completion because its console-reader thread can remain blocked while the evaluator runs. The Windows pseudo-pgid fast path still covers the controlled shell prompt; a child without that prompt settles on the silence tier (~3 s), as it does on macOS and Linux. The bash dialect retains Linux exact stdin-wait readiness.
