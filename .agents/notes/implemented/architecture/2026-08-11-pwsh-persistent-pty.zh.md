...
一个 backend、两种方言：`shellDialect: 'bash' | 'pwsh'`（默认 `'bash'`，存量部署逐字节不变）。有效 `shellPath`/`shellArgs` 按方言解析（bash `/bin/bash --noprofile --norc -i`；pwsh 经共享的 `dsh-pwsh-local` 解析器取 `-NoLogo -NoProfile`，保留交互宿主供子 REPL）。子环境去掉 bash 专属 `PS1`/`PROMPT_COMMAND` 标记并为 pwsh 加 `NO_COLOR`。pwsh 会话会用固定逻辑位置应答 PSReadLine 的标准光标位置请求，等待初始交互输出，然后只提交一次编码与提示符 bootstrap。静默结算后的后续 send 都是空观察；发布会话需要在同一个启动 deadline 内观察到 marker 所属提示符或完整的受控提示符行尾。两种方言发出相同的 BEL 终结 OSC `133;D;` 标记，并共享 `PROMPT_MARKER_PREFIX`、`CONTROLLED_PROMPT` 与精确尾部就绪；标记仍是载荷不被消费的就绪信号，不新增模型通知通道。[pwsh 就绪修正](../bug-fix/2026-08-20-pwsh-stdin-wait-readiness.md)拥有终端应答与完成证据的设计理由。
...
**PowerShell 在所有平台都使用提示符或静默就绪。** `pwsh` 方言会话不把线程级 stdin 等待视为命令完成，因为它的控制台读取线程可以在求值线程运行时保持阻塞。Windows 的伪 pgid 快路径仍覆盖受控 shell 提示符；没有该提示符的子进程按静默档结算（约 3s），与 macOS 和 Linux 相同。bash 方言保留 Linux 精确 stdin-wait 就绪。
