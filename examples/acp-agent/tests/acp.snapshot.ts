...
const ACOSMI_PROVIDER_CONFIG = fileURLToPath(
  new URL('./fixtures/acosmi-provider/cordis.yml', import.meta.url),
)
...
const SCENARIOS: Scenario[] = [
  ...
  // A deterministic account boundary drives the real Acosmi adapter through
  // Loader, ACP, agent-loop, and JSONL persistence. The fixture issues two
  // Harness tool calls, rejects split Anthropic user turns on continuation,
  // then yields managed search blocks that remain in replayState.
  {
    name: 'acosmi-managed-search',
    hasModelTurn: true,
    recorded: false,
    pinsHeader: true,
    headerClass: 'acosmi-managed-search',
    configPath: ACOSMI_PROVIDER_CONFIG,
  },
