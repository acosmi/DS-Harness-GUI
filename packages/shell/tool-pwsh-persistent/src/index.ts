...
function hasShellPromptTail(text: string): boolean {
...
function promptCompleted(result: TerminalSendResult): boolean {
  return hasShellPromptTail(result.viewport)
}
    ...
    const creation = (async () => {
      try {
        ...
        while (true) {
          ...
          const latest = ctx.terminals.read(owner, spawned.sessionId, {
            offset: 0,
            count: SCROLLBACK_PAGE_LINES,
          })
          if (promptCompleted(result) || hasShellPromptTail(latest.text)) break
          // Silence may settle while prompt installation is still running. An
          // empty observation keeps the next command from claiming its prompt.
        }
