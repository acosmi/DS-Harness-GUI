...
class StubTerminalSession implements TerminalBackendSession {
  ...
  constructor(mode: StubMode) {
    this.mode = mode
    this.scrollback = mode === 'init-idle-then-prompt' ? 'backend> ' : this.motd
  }
  ...
  startSend(request: TerminalSendRequest): TerminalSendOperation {
    ...
    if (this.mode === 'init-idle-then-prompt') {
      if (request.text.length > 0) throw new Error('wrapper submitted before private prompt')
      this.mode = 'normal'
...
describe('tool-pwsh-persistent', () => {
  it('observes the private prompt after initialization first settles as inferred idle', async () => {
    ...
    expect(stub.sessions[0]?.sentTexts[1]).toBe('')
    expect(stub.sessions[0]?.sentTexts[2]).toContain('__DSH_PERSISTENT_PWSH_START_')
  })
