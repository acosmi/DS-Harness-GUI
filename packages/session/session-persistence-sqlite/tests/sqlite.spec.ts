...
describe('SessionPersistenceSqlite schema ownership', () => {
  ...
  it('paces repeated busy journal-mode attempts', async () => {
    ...
    const db = await openDatabase(BusyDatabase, path, 'wal', 1_000)
    try {
      expect(attemptedAt).toHaveLength(3)
      expect(attemptedAt[1]! - attemptedAt[0]!).toBeGreaterThanOrEqual(9)
      expect(attemptedAt[2]! - attemptedAt[1]!).toBeGreaterThanOrEqual(9)
    } finally {
      db.close()
    }
  })
