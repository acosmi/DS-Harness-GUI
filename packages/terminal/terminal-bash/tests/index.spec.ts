...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        if (!initialized) throw new Error('setup submitted before the stock prompt')
        sent = request
        return {
  ...
  it('keeps waiting when a silence-settled setup echo contains the prompt literal', async () => {
    const ctx = new Context()
    await ctx.plugin(EmptySandbox)
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access', workspaceRoot: '/workspace' })
  ...
  it('rejects a truncated stock-prompt suffix as controlled prompt evidence', async () => {
    const ctx = new Context()
    await ctx.plugin(EmptySandbox)
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access', workspaceRoot: '/workspace' })
    const sends: TerminalSendRequest[] = []
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: sends.length === 1 ? 'dsh> ' : '',
            waitReason: sends.length === 1 ? 'inferred_idle' as const : 'stdin_read' as const,
      ...
      read: () => ({
        text: 'dsh> ', totalLines: 1, lineBegin: 0, lineEnd: 1, truncated: true,
      }),
    } as unknown as LocalPtySession
    const backend = new BashTerminalBackend(
      ctx,
      { ...config(), shellDialect: 'pwsh', shellPath: 'pwsh', maxReadBytes: 5 },
      async () => terminalHandle(),
      () => session,
    )
  ...
  it('bounds all silence-settled pwsh prompt observations with one deadline', async () => {
    vi.useFakeTimers()
    const ctx = new Context()
    await ctx.plugin(EmptySandbox)
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access', workspaceRoot: '/workspace' })
    ...
    const session = {
      ...
      startSend: () => ({
          ...
          setTimeout(() => {
            resolve({
              viewport: '',
              waitReason: closed ? 'session_exit' as const : 'inferred_idle' as const,
              sessionStatus: { kind: 'running' as const },
              truncated: false,
            })
          }, 10)
    ...
    const backend = new BashTerminalBackend(
      ctx,
      { ...config(), shellDialect: 'pwsh', shellPath: 'pwsh', timeoutMs: 35 },
      async () => terminalHandle(),
      () => session,
    )
  ...
  it('forwards the spawn signal into the pwsh bootstrap sends', async () => {
    ...
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access', workspaceRoot: '/workspace' })
    const sends: TerminalSendRequest[] = []
    const initializedWith: (AbortSignal | undefined)[] = []
    const session = {
      motd: '',
      initialize: async (signal?: AbortSignal) => { initializedWith.push(signal) },
      startSend: (request: TerminalSendRequest) => {
        sends.push(request)
        return {
          done: Promise.resolve({
            viewport: 'dsh> ', waitReason: 'inferred_idle' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
    ...
    expect(initializedWith).toEqual([signal])
