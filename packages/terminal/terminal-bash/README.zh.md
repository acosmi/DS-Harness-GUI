...
`shellDialect` 选择 shell 栈（默认 `bash`，或 `pwsh`）：它决定默认的 `shellPath`／`shellArgs`（bash 为 `--noprofile --norc -i`；pwsh 经共享的 `dsh-pwsh-local` 解析器得到 `-NoLogo -NoProfile`）与启动契约。bash 方言通过环境安装提示符（`PS1` 加 OSC `133;D;` 终结的 `PROMPT_COMMAND`）。`pwsh` 会话会用固定逻辑位置应答 PSReadLine 的标准光标位置请求，因为这个逐行后端不维护屏幕网格；启动流程会先等待由此产生的初始交互输出，再提交代码。随后，后端只提交一次 `prompt` 函数与共享的 `dsh-pwsh-local` 编码前缀，并用空观察操作等待受控提示符在同一个 `timeoutMs` deadline 内得到 marker 确认或占据完整行尾。编码前缀会在用户命令运行前把 `[Console]::OutputEncoding` 与 `$OutputEncoding` 钉为 UTF-8：会话解码路径按 UTF-8 读取 PTY 字节，而未钉住编码的控制台可能以宿主代码页输出非 ASCII 内容。pwsh 环境去掉 bash 专属标记并加 `NO_COLOR`；两种方言发出相同的 BEL 终结 OSC 标记，并共享其 sanitizer 与精确提示符尾部检查。

就绪检测结合以下机制：由前台状态验证的私有提示符标记、静默回退和绝对超时；只有 bash 方言可以额外接受提供方报告的前台 stdin 等待事实。`pwsh` 方言会话永远不把该 syscall 事实视为命令完成，因为它的控制台读取线程可以在另一个线程仍执行已提交命令时等待输入。只有最新自有标记之后的可打印尾部与受控提示符完全相等，标记才算就绪；即使 OSC 标记和提示符被拆到多个数据回调中也一样。因此，较早提示符之后的回显输入或输出无法使当前 send 完成。受控 bash `PROMPT_COMMAND` 会在每次输出提示符前重新设定 `PS1`，因此在 shell 内覆盖提示符不会使后续 send 退化到静默就绪。提供方写入前收集的提示符与静默证据，包括写入前前台检查仍在等待时收集的证据，都会在写入边界丢弃。如果 bash 在终端提供方发布其重新取得前台进程组的状态前打印标记，轮询会在普通静默上限之后再保留该候选状态 `handoffGraceMs`，使恰好同时发生的前台交接有机会胜出。因此，继承 `PROMPT_COMMAND` 的交互式子进程无法一直抑制推断空闲就绪直至绝对超时。未知的前台状态绝不会作为精确空闲的正向信号。对于 bash 方言，一次 send 之前就已存在的前台进程组 stdin 等待并不代表写入后就绪：必须先观察到同一进程组脱离该等待，之后再次进入等待才能使该次 send 完成；前台进程组发生变化则构成新的证据。尚未发布的启动过程中，回退路径要求已经观察到输出；零输出静默不能发布空会话，超时则拒绝 spawn。取消操作会关闭尚未发布的 shell，并以调用方提供的确切中止原因拒绝；`TerminalBackendCleanupError` 会单独保留清理失败。调用方的 signal 会转发给终端分配与就绪初始化；发布后，句柄负责其生命周期。未完成的终端控制序列使用独立于可打印输出读取的内部上限；超过上限后，系统会丢弃内容直到其终止符。格式错误的 UTF-8 终端输出使用替换字符；末尾的回车会跨回调保留，使拆分的 CRLF 合并为一个换行。
...
- bash 的精确 stdin 等待检测取决于已挂载的进程管理提供方；无法证明该状态的提供方使用提示符标记和静默／超时就绪机制。Windows 正是这样的提供方：shell pid 是伪前台进程组，没有精确的 stdin-wait 档，因此无标记的子进程按静默上限结算。
