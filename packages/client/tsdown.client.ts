...
/** Package roots whose client and Node build faces use this shared preset. */
const WORKSPACE_MANIFEST_GLOBS = [
  'packages/*/*/package.json',
  'downstream/packages/*/package.json',
] as const
...
export function clientBundle(
  ...
  return ({ env }) => {
    ...
    const client = clientConfig(id, clientEntry, options.inlineWireImports)
...
interface ClientBundleOptions {
  ...
  /** Product-owned generated wire modules that may be inlined without adding them to the shared runtime table. */
  readonly inlineWireImports?: readonly RegExp[]
}
...
function workspaceManifest(id: string): WorkspaceManifest {
  ...
  for (const manifestPath of globSync(WORKSPACE_MANIFEST_GLOBS, { cwd: REPOSITORY_ROOT })) {
  ...
  throw new Error(`tsdown: no client package manifest declares the name ${id}`)
}
...
function clientConfig(id: string, entry: string, inlineWireImports: readonly RegExp[] = []): UserConfig {
    ...
    plugins: [{
      ...
      resolveId(source: string) {
        ...
        if (INLINE_SAFE.test(source)
          ...
          || inlineWireImports.some(pattern => pattern.test(source))) return null // wire contribution: inline is the point
        throw new Error(
