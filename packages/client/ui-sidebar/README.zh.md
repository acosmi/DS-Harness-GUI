...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.brand`、`sidebar.workspaces`、`sidebar.settings` 与 `sidebar.footer.action` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。

`sidebar.brand` 是展开态 New Session 快捷按钮与收起态轨道切换按钮共用的可选 root-scoped single seat。owner 会请求 `variant: 'wordmark'` 或 `variant: 'mark'`；没有 registrant 时，外壳分别渲染 `BrandWordmark` 或 `FishLogo`。产品 bundle 只能替换这些视觉内容；两个按钮的行为与轨道中的面板图标交换仍由外壳持有。
