...
Sidebar shell plugin: the default wordmark, optional product-brand seat, New Session action, layout-owned collapse control, scroll-aware region seat, and bottom-pinned Settings seat. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.brand`, `sidebar.workspaces`, `sidebar.settings`, and `sidebar.footer.action` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.

`sidebar.brand` is an optional root-scoped single seat shared by the expanded New Session shortcut and the collapsed rail toggle. The owner requests `variant: 'wordmark'` or `variant: 'mark'`; without a registrant the shell renders `BrandWordmark` or `FishLogo` respectively. A product bundle may replace only those visuals while the shell retains both button behaviors and the rail's panel-toggle swap.
