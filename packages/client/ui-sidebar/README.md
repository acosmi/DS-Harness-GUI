...
The expanded New Session shortcut and collapsed rail toggle first render the optional root-scoped `sidebar.brand` single slot with a `wordmark` or `mark` variant. Its expanded fallback composes the independent `sidebar.brand.mark` and `sidebar.brand.name` slots, while its collapsed fallback renders the same mark slot. Without granular occupants, the shell uses the fish mark and a `DSH Local Build` label carrying the build's 7-character `DSH_CLIENT_COMMIT_HASH` badge. A deployment package can replace the complete variant or either granular value without replacing button behavior or rail geometry; declaration-aware `slots.inject()` lets such a package activate before or after the sidebar.
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the three brand slots, `sidebar.workspaces`, `sidebar.settings`, and `sidebar.footer.action` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
