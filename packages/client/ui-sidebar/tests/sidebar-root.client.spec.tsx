...
import type {
  SidebarBrandMarkOwnerProps, SidebarBrandNameOwnerProps, SidebarBrandOwnerProps,
...
function mountShell({
  ...
  brand?: (props: SidebarBrandOwnerProps) => ReactNode
  collapsed?: boolean
  width?: number
} = {}) {
  ...
  const root = () => (
    <SidebarRoot
      ...
      renderSlot={((
        key: string,
        owner: SidebarBrandMarkOwnerProps | SidebarBrandNameOwnerProps | SidebarBrandOwnerProps
          | SidebarFooterActionOwnerProps | SidebarSectionOwnerProps | SidebarSettingsOwnerProps,
        opts?: { fallback?: ReactNode },
      ) => {
        if (key === 'sidebar.brand') {
          return brand?.(owner as SidebarBrandOwnerProps) ?? opts?.fallback
        }
...
describe('SidebarRoot shell', () => {
  it('lets a product replace both sidebar brand variants without taking button behavior', () => {
    vi.useFakeTimers()
    const b = mountShell({
      brand: ({ variant }) => <span>Product {variant}</span>,
    })
    expect(screen.getByText('Product wordmark')).toBeTruthy()
    expect(screen.getAllByRole('button', { name: 'New session' })).toHaveLength(2)

    b.rerender({ collapsed: true })
    vi.advanceTimersByTime(200)
    b.rerender({})
    expect(screen.getByText('Product mark')).toBeTruthy()
  })
