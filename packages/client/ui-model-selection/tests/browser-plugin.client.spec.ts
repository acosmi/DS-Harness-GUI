...
import type { ModelProviderAccess } from '../src/client/service.ts'
...
describe('ui-model-selection dual entry', () => {
  ...
  it('blocks an unconfigured provider before selection while leaving other routes available', async () => {
    const b = await bench()
    b.mint('s1')
    const face = b.seat().inject!(sid('s1'))
    face.load()
    await Promise.resolve()
    await Promise.resolve()
    const access = createSnapshotStore<ModelProviderAccess>({
      status: 'blocked',
      reason: 'Configure the DeepSeek API key or choose an Acosmi account model.',
    })
    const dispose = b.ctx.modelDirectories.registerProviderAccess('deepseek-official', access)

    expect(b.blockOf('s1')?.reason).toBe('Configure the DeepSeek API key or choose an Acosmi account model.')
    await expect(face.select({ provider: 'deepseek-official', model: 'deepseek-v4-pro' })).resolves.toBe(false)
    expect(b.calls.select).toBe(0)
    expect(face.directory.getSnapshot().error).toMatch(/Configure the DeepSeek API key/)

    access.set({ status: 'available' })
    expect(b.blockOf('s1')).toBeUndefined()
    expect(await face.select({ provider: 'deepseek-official', model: 'deepseek-v4-pro' })).toBe(true)
    expect(b.calls.select).toBe(1)
    dispose()
  })

  it('does not interpret inherited record properties as provider blocks', async () => {
    const b = await bench()
    b.mint('s1')
    const face = b.seat().inject!(sid('s1'))

    await expect(face.select({ provider: 'toString', model: 'prototype-safe' })).resolves.toBe(true)
    expect(b.calls.select).toBe(1)
  })
