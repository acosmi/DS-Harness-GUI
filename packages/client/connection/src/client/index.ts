...
import { installedConnectionCarrier } from '../carrier.ts'
...
export { installConnectionCarrier } from '../carrier.ts'
export type { ConnectionCarrier } from '../carrier.ts'
...
export function apply(ctx: Context): void {
  ...
  const carrier = installedConnectionCarrier()
  const api: IApiClient = fixtureClient ?? carrier?.api ?? new WebApiClient()
  const rpc = fixtureClient?.rpc ?? carrier?.rpc ?? createWebConnectionRpc()
  let started = false
  ...
  const handle: ConnectionHandle = {
    api,
    isLoopback: fixtureClient !== undefined
      ? true
      : carrier?.isLoopback ?? (pageLocation === undefined || isLoopbackHostname(pageLocation.hostname)),
    hostDescription: {
