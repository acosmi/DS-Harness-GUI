...
describe('WorkspaceAnalyzer', { timeout: 60_000 }, () => {
  ...
  it('discovers product extensions under the downstream package root', () => {
    const root = copyFixture('typert-downstream-package-')
    const packageRoot = join(root, 'downstream/packages/product-account')
    mkdirSync(join(packageRoot, 'src'), { recursive: true })
    writeFileSync(join(packageRoot, 'package.json'), `${JSON.stringify({
      name: '@fixture/product-account',
      private: true,
      type: 'module',
      exports: {
        '.': { types: './lib/types/index.d.ts', default: './lib/index.js' },
    ...
    const aggregatePath = join(root, 'tsconfig.host.json')
    const aggregate = JSON.parse(readFileSync(aggregatePath, 'utf8')) as { references: { path: string }[] }
    aggregate.references.push({ path: './downstream/packages/product-account' })
    writeFileSync(aggregatePath, `${JSON.stringify(aggregate, null, 2)}\n`)

    expect(new WorkspaceAnalyzer({ root }).discoverPackages()).toContainEqual({
      package: '@fixture/product-account',
      root: 'downstream/packages/product-account',
      faces: ['host'],
    })
  })
