...
/**
 * Target-native payloads repeated in the desktop app manifest so
 * electron-builder retains them. The root notice lists their parent direct
 * dependencies; the selected payloads remain subject to the packaged-artifact audit.
 */
const DESKTOP_PLATFORM_PAYLOADS = new Set([
...
/**
 * Whether one dependency is a reviewed desktop platform payload whose parent
 * package owns the direct-dependency notice entry.
 * @param name - exact npm package identity.
 * @returns true only for the desktop manifest's reviewed payload pins.
 */
export function isDesktopPlatformPayload(name: string): boolean {
  return DESKTOP_PLATFORM_PAYLOADS.has(name)
}
...
function collectNpmDeps(): ExternalDep[] {
  ...
  return [...tierExternalDeps(manifests, names)]
    .filter(([name]) => !FIRST_PARTY.has(name) && !isDesktopPlatformPayload(name))
