...
if (failures.length > 0) {
  const displayPath = relative(root, runtimeManifestPath)
  console.error(`verify-runtime-closure: required workspace peers are missing from ${displayPath} dependencies:`)
...
async function loadWorkspacePackages(): Promise<Map<string, WorkspacePackage>> {
  const paths = globSync([
    'downstream/apps/*/package.json',
    'downstream/bundles/*/package.json',
