...
describe('desktop platform payload disclosure', () => {
  it('collapses only the reviewed native pins into their disclosed parent packages', () => {
    expect(isDesktopPlatformPayload('@img/sharp-darwin-arm64')).toBe(true)
    expect(isDesktopPlatformPayload('@koromix/koffi-win32-x64')).toBe(true)
    expect(isDesktopPlatformPayload('node-addon-require-builtin-darwin-x64')).toBe(true)
    expect(isDesktopPlatformPayload('sharp')).toBe(false)
    expect(isDesktopPlatformPayload('@img/unreviewed-platform-payload')).toBe(false)
  })
})
