...
function clientConfigs(id = REQUESTING_PACKAGE, inlineWireImports: readonly RegExp[] = []) {
  return clientBundle(id, ['lib/types/index.js', 'lib/types/invariant.js'], { inlineWireImports })(
...
function purityResolveId(id = REQUESTING_PACKAGE, inlineWireImports: readonly RegExp[] = []): ResolveId {
  ...
  const configs = clientConfigs(id, inlineWireImports)
...
describe('client bundle purity gate', () => {
  ...
  it('lets one bundle extend the generated wire-module allowlist without admitting adjacent values', () => {
    const productWire = purityResolveId(REQUESTING_PACKAGE, [
      /^@deepseek-ai\/dsh-product-account\/remote$/,
    ])

    expect(productWire('@deepseek-ai/dsh-product-account/remote')).toBeNull()
    expect(() => productWire('@deepseek-ai/dsh-product-account')).toThrow(/purity/)
    expect(() => productWire('@deepseek-ai/dsh-product-account/remote/nested')).toThrow(/purity/)
  })
...
describe('client bundle module requests', () => {
  it('reads private downstream manifests and keeps their wire-only dependencies inside the bundle', () => {
    const config = clientConfigs('@acosmi/dsh-api-remotes-acosmi', [
      /^@acosmi\/dsh-account-acosmi\/remote$/,
    ])[0]
    const deps = config?.deps as {
      neverBundle: (specifier: string) => boolean
      alwaysBundle: (specifier: string) => boolean
    }

    expect(deps.neverBundle('@acosmi/dsh-account-acosmi/remote')).toBe(false)
    expect(deps.alwaysBundle('@acosmi/dsh-account-acosmi/remote')).toBe(true)
  })
