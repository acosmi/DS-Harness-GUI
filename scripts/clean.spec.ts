...
describe('RepositoryCleaner', () => {
  ...
  it('removes the shared lib root for a nested compiler-face output', async () => {
    const root = fixture()
    addProject(root, 'products/remotes', 'lib/types/client')
    write(join(root, 'products/remotes/lib/types/client/index.js'))
    write(join(root, 'products/remotes/lib/client.js'))

    await new RepositoryCleaner(root).clean()

    expect(existsSync(join(root, 'products/remotes/lib'))).toBe(false)
    expect(existsSync(join(root, 'products/remotes/src/index.ts'))).toBe(true)
  })
