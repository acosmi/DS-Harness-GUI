...
function buildOutputRoot(typesDirectory: string, nativeEntryOutput: string): string | undefined {
  if (typesDirectory === nativeEntryOutput) return typesDirectory

  let ancestor = typesDirectory
  while (basename(ancestor) !== 'types' && dirname(ancestor) !== ancestor) {
    ancestor = dirname(ancestor)
  }
  return basename(ancestor) === 'types' && basename(dirname(ancestor)) === 'lib'
    ? dirname(ancestor)
    : undefined
}
...
export class RepositoryCleaner {
  ...
  private async plan(): Promise<string[]> {
    ...
    // Each emitting project declares an outDir at or below lib/types; the shared
    // parent lib also owns sibling runtime bundles, so the complete output root is removed.
  ...
  private buildOutputDirectories(): string[] {
    ...
    while (pending.length > 0) {
      ...
      if (parsed.options.outDir !== undefined) {
        ...
        const outputDirectory = buildOutputRoot(typesDirectory, nativeEntryOutput)
        if (outputDirectory === undefined) {
          throw new Error(`clean: expected TypeScript outDir at or below /lib/types: ${repositoryPath(this.root, typesDirectory)}`)
        }
